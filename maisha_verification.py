...
# CONFIGURATION
# ============================================================
CLIENT_THRESHOLD = 60.0   # Override broken API threshold (API reports 70% but behaves like ~100%)

# Set up logging
log_filename = f'maisha_verification_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
...
def clean_base64_string(b64_string) -> str:
    """Clean base64 string by removing data URI prefixes and whitespace."""
    if not b64_string:
        return ""

    # Handle if it's a bytes-like object from Oracle CLOB
    if hasattr(b64_string, 'read'):
        b64_string = b64_string.read()
...
def validate_base64_image(b64_string: str) -> bool:
    ...
    try:
        ...
        if image_bytes[:3] != b'\xff\xd8\xff':
            logger.warning(f"Not a JPEG. First bytes: {image_bytes[:10].hex()}")
...
class MaishaVerificationTester:
    def __init__(self,
        ...
        # Create image export directories
        if self.save_disagreement_images:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.images_dir = Path(f"disagreement_images_{timestamp}")
            self.images_dir.mkdir(exist_ok=True)

            # Category 1: GBG Approved, AWS Rejected (False Negatives - legitimate customers AWS is blocking)
            self.fn_dir = self.images_dir / "1_GBG_APPROVED_AWS_REJECTED"
            self.fn_dir.mkdir(exist_ok=True)

            # Category 2: GBG Rejected, AWS Approved (False Positives - possible fraud AWS is letting through)
            self.fp_dir = self.images_dir / "2_GBG_REJECTED_AWS_APPROVED"
            self.fp_dir.mkdir(exist_ok=True)

            # Category 3: Both Approved samples (sanity check)
            self.both_pass_dir = self.images_dir / "3_BOTH_APPROVED_samples"
            self.both_pass_dir.mkdir(exist_ok=True)

            # Category 4: Both Rejected samples
            self.both_fail_dir = self.images_dir / "4_BOTH_REJECTED_samples"
            self.both_fail_dir.mkdir(exist_ok=True)

            self.both_pass_saved = 0
            self.both_fail_saved = 0
            self.SAMPLE_LIMIT = 10  # Save up to 10 samples for agreement cases
        ...
        logger.info("=" * 70)
        logger.info("MAISHA CARD VERIFICATION - AWS vs GBG COMPARISON")
        logger.info("=" * 70)
        logger.info(f"  API Endpoint:          {api_base_url}")
        logger.info(f"  API Key:               {api_key[:20]}...")
        logger.info(f"  GBG Comparison:        {'ENABLED' if compare_with_gbg else 'DISABLED'}")
        logger.info(f"  Client Threshold:      {client_threshold}% (overriding API's broken threshold)")
        logger.info(f"  Save Images:           {'YES' if save_disagreement_images else 'NO'}")
        logger.info("=" * 70)
    ...
    def save_image_pair(self, card_base64: str, face_base64: str,
                        filename_prefix: str, directory: Path,
                        metadata: Dict = None):
        """Save card + face image pair with optional metadata JSON."""
        try:
            card_data = base64.b64decode(card_base64)
            card_path = directory / f"{filename_prefix}_CARD.jpg"
            with open(card_path, 'wb') as f:
            ...
            logger.warning(f"Failed to save image pair: {str(e)}")
            return False
    ...
    def test_network_connectivity(self):
        """Test basic TCP connectivity to Oracle database."""
        import socket
    ...
    def fetch_maisha_records(self, limit: int = None, gbg_filter: str = None) -> List[Dict]:
        """
        Fetch records for comparison.

        GBG filter options:
          'APPROVED'   -> LIVELINESS_CHECK=1 AND HIGH_LEVEL_RESULT='Passed'  (GBG approved face verification)
          'REJECTED'   -> LIVELINESS_CHECK=1 AND HIGH_LEVEL_RESULT!='Passed' (GBG rejected face verification)
          'ALL'        -> LIVELINESS_CHECK=1 (both approved and rejected - recommended for full comparison)
          None         -> Same as 'ALL'
        """
        try:
            logger.info(f"Connecting to Oracle database: {self.oracle_config['dsn']}")
            ...
            # -------------------------------------------------------
            # CORRECT QUERY:
            # - LIVELINESS_CHECK = 1  → Customer actually completed face verification
            # - HIGH_LEVEL_RESULT     → GBG's actual verdict on face match ('Passed' = approved)
            # - o.AWS_IMAGE           → Selfie captured during onboarding
            # - k.KYC_IDFRONT_CAPTURE → ID card photo
            # -------------------------------------------------------
            query = """
                SELECT
                    blob_to_clob(o.AWS_IMAGE)          AS FACE_IMAGE_BASE64,
                    blob_to_clob(k.KYC_IDFRONT_CAPTURE) AS CARD_IMAGE_BASE64,
                    k.KYC_ID_NO,
                    k.SESSION_ID,
                    sm.JOURNEY_ID                       AS GBG_JOURNEY_ID,
                    sm.LIVELINESS_CHECK                 AS GBG_LIVELINESS,
                    sm.OCR_CHECK_MISMATCH               AS GBG_OCR_CHECK,
                    k.HIGH_LEVEL_RESULT                 AS GBG_RESULT,
                    sm.CREATED_DATE                     AS ONBOARDING_DATE
                FROM MA.SELF_ONBOARDING_TRACKER_AWS o
                JOIN MA.SELF_ONBOARDING_TRACKER_KYC k
                    ON o.SESSION_ID = k.SESSION_ID
            ...
            # Apply GBG result filter
            if gbg_filter == 'APPROVED':
                query += " AND k.HIGH_LEVEL_RESULT = 'Passed'"
                logger.info("  GBG Filter: Only GBG APPROVED (HIGH_LEVEL_RESULT='Passed')")
            elif gbg_filter == 'REJECTED':
                query += " AND (k.HIGH_LEVEL_RESULT != 'Passed' OR k.HIGH_LEVEL_RESULT IS NULL)"
                logger.info("  GBG Filter: Only GBG REJECTED (HIGH_LEVEL_RESULT!='Passed')")
            else:
                logger.info("  GBG Filter: ALL (both approved and rejected) - RECOMMENDED")
            ...
            logger.info(f"  Fetching up to {limit if limit else 'ALL'} records...")
            logger.info("=" * 70)
            ...
            gbg_approved_count = 0
            gbg_rejected_count = 0

            for idx, row in enumerate(cursor, 1):
                try:
                    face_image_clob = row[0]
                    card_image_clob = row[1]
                    kyc_id         = row[2]
                    session_id     = row[3]
                    gbg_journey_id = row[4]
                    gbg_liveliness = row[5]
                    gbg_ocr_check  = row[6]
                    gbg_result     = row[7]
                    onboarding_date = row[8]

                    if session_id in session_ids_seen:
                        skip_reasons['duplicate_session'] += 1
                        continue

                    session_ids_seen.add(session_id)

                    face_image_data = face_image_clob.read() if face_image_clob else None
                    card_image_data = card_image_clob.read() if card_image_clob else None

                    if not face_image_data:
                        skip_reasons['missing_face_image'] += 1
                        self.skipped_records.append({'session_id': session_id, 'reason': 'missing_face_image'})
                        continue

                    if not card_image_data:
                        skip_reasons['missing_card_image'] += 1
                        self.skipped_records.append({'session_id': session_id, 'reason': 'missing_card_image'})
                        continue

                    face_clean = clean_base64_string(face_image_data)
                    card_clean = clean_base64_string(card_image_data)

                    if len(face_clean) < 100:
                        skip_reasons['face_too_small'] += 1
                        continue

                    if len(card_clean) < 100:
                        skip_reasons['card_too_small'] += 1
                        continue

                    try:
                        base64.b64decode(face_clean[:100])
                        base64.b64decode(card_clean[:100])
                    except Exception:
                        skip_reasons['invalid_base64'] += 1
                        continue

                    # GBG verdict
                    gbg_verified = (gbg_result == 'Passed')
                    if gbg_verified:
                        gbg_approved_count += 1
                    else:
                        gbg_rejected_count += 1

                    record = {
                        'face_image_base64': face_clean,
                        'card_image_base64': card_clean,
                        'KYC_ID_NO':         kyc_id,
                        'SESSION_ID':        session_id,
                        'GBG_JOURNEY_ID':    gbg_journey_id,
                        'GBG_LIVELINESS':    gbg_liveliness,
                        'GBG_OCR_CHECK':     gbg_ocr_check,
                        'GBG_RESULT':        gbg_result,
                        'GBG_VERIFIED':      gbg_verified,
                    ...
                    if idx <= 5:
                        logger.info(f"  Record {idx:2d} | Session: {session_id[:20]}... | KYC: {kyc_id}")
                        logger.info(f"             | GBG: {'APPROVED' if gbg_verified else 'REJECTED'} ({gbg_result})")
                        logger.info(f"             | Card: {len(card_clean)} chars | Face: {len(face_clean)} chars")
                    elif idx % 20 == 0:
                        logger.info(f"  Processed {idx} records, kept {len(records)}...")
            ...
            logger.info("=" * 70)
            logger.info(f"[SUCCESS] Fetched {len(records)} valid records")
            logger.info(f"  GBG Approved (HIGH_LEVEL_RESULT='Passed'): {gbg_approved_count:,}")
            logger.info(f"  GBG Rejected:                              {gbg_rejected_count:,}")

            if skip_reasons:
                logger.info("\n[INFO] Skipped Records:")
    ...
    def verify_batch_using_client(self, records: List[Dict], batch_num: int) -> List[Dict]:
        """Verify a batch using the Maisha API with client-side threshold override."""
        logger.info(f"Preparing batch {batch_num} with {len(records)} records...")
        ...
        for record in records:
            verifications.append({
                "id":           record['SESSION_ID'],
                "source_image": record['card_image_base64'],   # ID card (source)
                "target_image": record['face_image_base64'],   # Selfie (target)
                "reference_id": str(record['KYC_ID_NO'])
            })
        ...
        try:
            ...
            # Calculate using CLIENT threshold (API threshold is broken)
            client_passed = sum(1 for r in batch_result.results
                               if r.get('similarity_score', 0) >= self.client_threshold)
            logger.info(f"  Passed (client {self.client_threshold}%): {client_passed}")
            logger.info(f"  API 'match' field (BROKEN/IGNORED):       {batch_result.passed}")
            ...
            for idx, api_result in enumerate(batch_result.results):
                ...
                # -------------------------------------------------------
                # CLIENT-SIDE THRESHOLD - Ignore API's broken match field
                # -------------------------------------------------------
                aws_verified = similarity_score >= self.client_threshold

                combined = {
                    'record_index':         records[idx]['record_index'],
                    'session_id':           records[idx]['SESSION_ID'],
                    'kyc_id_no':            records[idx]['KYC_ID_NO'],
                    'onboarding_date':      records[idx].get('ONBOARDING_DATE'),

                    # AWS results
                    'aws_verified':         aws_verified,
                    'aws_api_match':        api_result.get('match', False),  # Original (broken)
                    'aws_similarity_score': similarity_score,
                    'aws_threshold':        self.client_threshold,
                    'aws_comparison_method': api_result.get('comparison_method'),
                    'aws_comparison_id':    api_result.get('comparison_id'),
                    ...
                    # GBG results
                    'gbg_journey_id':       records[idx].get('GBG_JOURNEY_ID'),
                    'gbg_liveliness':       records[idx].get('GBG_LIVELINESS'),
                    'gbg_ocr_check':        records[idx].get('GBG_OCR_CHECK'),
                    'gbg_result':           records[idx].get('GBG_RESULT'),
                    'gbg_verified':         records[idx].get('GBG_VERIFIED', False),
                ...
                combined['both_rejected'] = (not gbg_verified and not aws_verified)
                combined['false_negative'] = (gbg_verified and not aws_verified)  # GBG approved, AWS rejected
                combined['false_positive'] = (not gbg_verified and aws_verified)  # GBG rejected, AWS approved

                # -------------------------------------------------------
                # SAVE IMAGES
                # -------------------------------------------------------
                if self.save_disagreement_images and not api_result.get('error'):
                    score_str = f"{similarity_score:.1f}pct"
                    session_short = records[idx]['SESSION_ID'][:8]
                    metadata = {
                        'session_id':         records[idx]['SESSION_ID'],
                        'kyc_id':             records[idx]['KYC_ID_NO'],
                    ...
                    if combined['false_negative']:
                        # GBG APPROVED, AWS REJECTED → False Negatives (legitimate customers blocked)
                        prefix = f"FN_{score_str}_{session_short}"
                        self.save_image_pair(
                            records[idx]['card_image_base64'],
                            records[idx]['face_image_base64'],
                            prefix, self.fn_dir, metadata
                        )

                    elif combined['false_positive']:
                        # GBG REJECTED, AWS APPROVED → False Positives (potential fraud let through)
                        prefix = f"FP_{score_str}_{session_short}"
                        self.save_image_pair(
                            records[idx]['card_image_base64'],
                            records[idx]['face_image_base64'],
                            prefix, self.fp_dir, metadata
                        )

                    elif combined['both_verified'] and self.both_pass_saved < self.SAMPLE_LIMIT:
                        # Both approved - save samples
                        prefix = f"PASS_{score_str}_{session_short}"
                        self.save_image_pair(
                            records[idx]['card_image_base64'],
                            records[idx]['face_image_base64'],
                            prefix, self.both_pass_dir, metadata
                        )
                        self.both_pass_saved += 1

                    elif combined['both_rejected'] and self.both_fail_saved < self.SAMPLE_LIMIT:
                        # Both rejected - save samples
                        prefix = f"FAIL_{score_str}_{session_short}"
                        self.save_image_pair(
                            records[idx]['card_image_base64'],
                            records[idx]['face_image_base64'],
                            prefix, self.both_fail_dir, metadata
                        )
                        self.both_fail_saved += 1
    ...
    def run_batch_test(self, batch_size: int = 5, total_limit: int = None,
                       gbg_filter: str = None) -> List[Dict]:
        """Run the full batch verification test."""
        logger.info("=" * 70)
        ...
        logger.info(f"  GBG Filter:      {gbg_filter if gbg_filter else 'ALL (approved + rejected)'}")
        logger.info(f"  Threshold:       {self.client_threshold}%")
        logger.info("=" * 70)
        ...
        records = self.fetch_maisha_records(limit=total_limit, gbg_filter=gbg_filter)
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                ...
                for idx, result in enumerate(batch_results):
                    record_num = i + idx + 1
                    score      = result.get('aws_similarity_score', 0)
                    gbg_ok     = result.get('gbg_verified', False)
                    aws_ok     = result.get('aws_verified', False)
                    agree      = result.get('agreement', False)

                    if result.get('aws_error'):
                        logger.warning(f"  {record_num:3d}. [ERROR] {result['aws_error'][:60]}")
                    else:
                        label = ""
                        if result.get('false_negative'): label = " ← FALSE NEGATIVE"
                        elif result.get('false_positive'): label = " ← FALSE POSITIVE"

                        logger.info(
                            f"  {record_num:3d}. "
                            f"GBG:{'✓' if gbg_ok else '✗'} "
                            f"AWS:{'✓' if aws_ok else '✗'} "
                            f"Agree:{'✓' if agree else '✗'} "
                            f"| Score: {score:5.2f}%{label}"
                        )
    ...
    def analyze_results(self) -> Dict:
        ...
        total = len(self.results)
        errors = [r for r in self.results if r.get('aws_error')]
        valid  = [r for r in self.results if not r.get('aws_error')]

        aws_verified    = sum(1 for r in valid if r.get('aws_verified'))
        gbg_verified    = sum(1 for r in valid if r.get('gbg_verified'))
        both_verified   = sum(1 for r in valid if r.get('both_verified'))
        both_rejected   = sum(1 for r in valid if r.get('both_rejected'))
        false_negatives = sum(1 for r in valid if r.get('false_negative'))
        false_positives = sum(1 for r in valid if r.get('false_positive'))
        agreement       = sum(1 for r in valid if r.get('agreement'))

        scores = [r.get('aws_similarity_score', 0) for r in valid]
        avg_score = sum(scores) / len(scores) if scores else 0
        max_score = max(scores) if scores else 0
        min_score = min(scores) if scores else 0
        ...
        # Score distribution for GBG-approved records only
        gbg_approved_scores = [r.get('aws_similarity_score', 0) for r in valid if r.get('gbg_verified')]
        gbg_approved_dist = {
            '90-100%':   sum(1 for s in gbg_approved_scores if s >= 90),
            '80-89%':    sum(1 for s in gbg_approved_scores if 80 <= s < 90),
            '70-79%':    sum(1 for s in gbg_approved_scores if 70 <= s < 80),
            '60-69%':    sum(1 for s in gbg_approved_scores if 60 <= s < 70),
            'Below 60%': sum(1 for s in gbg_approved_scores if s < 60),
        }
        ...
        return {
            ...
            # AWS
            'aws_verified_count':    aws_verified,
            'aws_not_verified':      len(valid) - aws_verified,
            'aws_verification_rate': (aws_verified / len(valid) * 100) if valid else 0,

            # GBG
            'gbg_verified_count':    gbg_verified,
            'gbg_not_verified':      len(valid) - gbg_verified,
            'gbg_verification_rate': (gbg_verified / len(valid) * 100) if valid else 0,

            # Agreement
            'both_verified_count':   both_verified,
            'both_rejected_count':   both_rejected,
            'false_negatives':       false_negatives,   # GBG ✓ AWS ✗
            'false_positives':       false_positives,   # GBG ✗ AWS ✓
            'agreement_count':       agreement,
            'agreement_rate':        (agreement / len(valid) * 100) if valid else 0,
            'false_negative_rate':   (false_negatives / gbg_verified * 100) if gbg_verified else 0,
            'false_positive_rate':   (false_positives / (len(valid) - gbg_verified) * 100)
                                     if (len(valid) - gbg_verified) > 0 else 0,
            ...
            'gbg_approved_score_distribution': gbg_approved_dist,
            'avg_score_gbg_approved': sum(gbg_approved_scores) / len(gbg_approved_scores)
                                      if gbg_approved_scores else 0,
        }
    ...
    def export_csv(self, output_file: str = None) -> str:
        ...
        columns = [
            'record_index', 'session_id', 'kyc_id_no', 'onboarding_date',
            'gbg_journey_id', 'gbg_liveliness', 'gbg_ocr_check',
            'gbg_result', 'gbg_verified',
            'aws_verified', 'aws_api_match', 'aws_similarity_score', 'aws_threshold',
            'aws_comparison_method', 'aws_comparison_id',
            'agreement', 'both_verified', 'both_rejected',
    ...
    def export_skipped_records(self, output_file: str = None) -> str:
        """Export skipped records to CSV for data quality review."""
        if not self.skipped_records:
            return None

        if not output_file:
            output_file = f"maisha_skipped_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        logger.info(f"[SUCCESS] Skipped records exported: {output_file}")
        return output_file
    ...
    def print_summary(self, analysis: Dict):
        ...
        print("\n" + "=" * 70)
        print("MAISHA CARD VERIFICATION - AWS vs GBG COMPARISON RESULTS")
        print("=" * 70)

        print(f"\nTOTAL RECORDS TESTED: {analysis.get('total_tests', 0):,}")
        print(f"  Valid:   {analysis.get('valid_tests', 0):,}")
        print(f"  Errors:  {analysis.get('error_count', 0):,}")

        print(f"\nGBG RESULTS (ground truth):")
        print(f"  ✓ Approved: {analysis.get('gbg_verified_count', 0):,} ({analysis.get('gbg_verification_rate', 0):.1f}%)")
        print(f"  ✗ Rejected: {analysis.get('gbg_not_verified', 0):,}")

        print(f"\nAWS RESULTS (threshold: {self.client_threshold}%):")
        print(f"  ✓ Verified:     {analysis.get('aws_verified_count', 0):,} ({analysis.get('aws_verification_rate', 0):.1f}%)")
        print(f"  ✗ Not Verified: {analysis.get('aws_not_verified', 0):,}")

        print(f"\nAGREEMENT ANALYSIS:")
        print(f"  Overall Agreement Rate:    {analysis.get('agreement_rate', 0):.1f}%")
        print(f"  ✓✓ Both Approved:          {analysis.get('both_verified_count', 0):,}")
        print(f"  ✗✗ Both Rejected:          {analysis.get('both_rejected_count', 0):,}")
        print(f"  ✓✗ FALSE NEGATIVE (GBG✓ AWS✗): {analysis.get('false_negatives', 0):,}  "
              f"({analysis.get('false_negative_rate', 0):.1f}% of GBG approved)")
        print(f"  ✗✓ FALSE POSITIVE (GBG✗ AWS✓): {analysis.get('false_positives', 0):,}  "
              f"({analysis.get('false_positive_rate', 0):.1f}% of GBG rejected)")

        print(f"\nAWS SIMILARITY SCORES (all records):")
        print(f"  Average:  {analysis.get('avg_similarity_score', 0):.2f}%")
        print(f"  Maximum:  {analysis.get('max_similarity_score', 0):.2f}%")
        print(f"  Minimum:  {analysis.get('min_similarity_score', 0):.2f}%")

        print(f"\nSCORE DISTRIBUTION (all records):")
        for range_name, count in analysis.get('score_distribution', {}).items():
            print(f"  {range_name:12s}: {count:4d} records")

        print(f"\nSCORE DISTRIBUTION (GBG-approved records only):")
        print(f"  Average AWS score for GBG-approved: {analysis.get('avg_score_gbg_approved', 0):.2f}%")
        for range_name, count in analysis.get('gbg_approved_score_distribution', {}).items():
            print(f"  {range_name:12s}: {count:4d} records")

        if self.save_disagreement_images:
            print(f"\nIMAGES SAVED TO: {self.images_dir}/")
            print(f"  1_GBG_APPROVED_AWS_REJECTED/ → {self.fn_dir.name}  (False Negatives)")
            print(f"  2_GBG_REJECTED_AWS_APPROVED/ → {self.fp_dir.name}  (False Positives)")
            print(f"  3_BOTH_APPROVED_samples/     → {self.both_pass_saved} samples")
            print(f"  4_BOTH_REJECTED_samples/     → {self.both_fail_saved} samples")

        print("=" * 70 + "\n")

    def generate_executive_summary(self, analysis: Dict, output_file: str = None) -> str:
        """Generate a business-friendly executive summary for management."""
        if not output_file:
            output_file = f"executive_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"

        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("CO-OPERATIVE BANK OF KENYA\n")
            f.write("Maisha Card Verification System\n")
            f.write("AWS Rekognition vs GBG Performance Comparison\n")
            f.write("=" * 80 + "\n\n")

            f.write(f"Report Date:     {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Database:        {self.oracle_config['dsn']}\n")
            f.write(f"Records Tested:  {analysis.get('total_tests', 0):,}\n")
            f.write(f"AWS Threshold:   {self.client_threshold}% (client-side)\n")
            f.write(f"GBG Verdict:     HIGH_LEVEL_RESULT = 'Passed'\n\n")
            ...
            f.write(f"Overall Agreement Rate:    {agreement_rate:.1f}%\n")
            f.write(f"AWS Verification Rate:     {analysis.get('aws_verification_rate', 0):.1f}%\n")
            f.write(f"GBG Verification Rate:     {analysis.get('gbg_verification_rate', 0):.1f}%\n\n")

            f.write("Agreement Breakdown:\n")
            f.write(f"  Both Approved:               {analysis.get('both_verified_count', 0):,} records\n")
            f.write(f"  Both Rejected:               {analysis.get('both_rejected_count', 0):,} records\n")
            f.write(f"  GBG Approved / AWS Rejected: {analysis.get('false_negatives', 0):,} records "
                    f"(False Negatives - {fn_rate:.1f}%)\n")
            f.write(f"  GBG Rejected / AWS Approved: {analysis.get('false_positives', 0):,} records "
                    f"(False Positives - {fp_rate:.1f}%)\n\n")

            f.write("AWS Score Statistics:\n")
            f.write(f"  Overall Average:              {analysis.get('avg_similarity_score', 0):.2f}%\n")
            f.write(f"  GBG-Approved Average:         {analysis.get('avg_score_gbg_approved', 0):.2f}%\n")
            f.write(f"  Maximum:                      {analysis.get('max_similarity_score', 0):.2f}%\n")
            f.write(f"  Minimum:                      {analysis.get('min_similarity_score', 0):.2f}%\n\n")

            f.write("Score Distribution (GBG-approved records):\n")
            for range_name, count in analysis.get('gbg_approved_score_distribution', {}).items():
                f.write(f"  {range_name:12s}: {count:4d} records\n")
            f.write("\n")
            ...
            if agreement_rate >= 95:
                f.write(f"✓ EXCELLENT: AWS agrees with GBG at {agreement_rate:.1f}%.\n")
                f.write("  Recommendation: AWS is ready for production deployment.\n\n")
            elif agreement_rate >= 90:
                f.write(f"✓ GOOD: AWS agrees with GBG at {agreement_rate:.1f}%.\n")
                f.write("  Recommendation: Review mismatch cases before production deployment.\n\n")
            elif agreement_rate >= 80:
                f.write(f"⚠ MODERATE: AWS agrees with GBG at {agreement_rate:.1f}%.\n")
                f.write("  Recommendation: Investigate disagreements and tune threshold.\n\n")
            else:
                f.write(f"✗ LOW: AWS agrees with GBG at only {agreement_rate:.1f}%.\n")
                f.write("  Recommendation: Further investigation required before deployment.\n\n")

            if fn_rate > 5:
                f.write(f"⚠ HIGH FALSE NEGATIVE RATE ({fn_rate:.1f}%)\n")
                f.write("  Impact: Legitimate customers being rejected by AWS.\n")
                f.write("  Recommendation: Lower the similarity threshold.\n\n")

            if fp_rate > 2:
                f.write(f"⚠ HIGH FALSE POSITIVE RATE ({fp_rate:.1f}%)\n")
                f.write("  Impact: Potentially fraudulent applications being approved.\n")
                f.write("  Recommendation: Raise threshold or add manual review for flagged cases.\n\n")

            f.write("⚠ API THRESHOLD BUG NOTE:\n")
            f.write("  The Maisha API reports 70% threshold in the health check but\n")
            f.write("  only returns match=True for scores >99.85%. This report uses\n")
            f.write(f"  a client-side threshold of {self.client_threshold}% to override this.\n")
            f.write("  Action required: Contact Maisha vendor to fix the threshold.\n\n")

            if self.save_disagreement_images:
                fn_count = len(list(self.fn_dir.glob('*_INFO.json')))
                fp_count = len(list(self.fp_dir.glob('*_INFO.json')))
                f.write("Image Evidence:\n")
                f.write(f"  False Negative images saved: {fn_count} pairs\n")
                f.write(f"  False Positive images saved: {fp_count} pairs\n")
                f.write(f"  Location: {self.images_dir}/\n\n")

            f.write("=" * 80 + "\n")
            f.write("END OF REPORT\n")
            f.write("=" * 80 + "\n")

        logger.info(f"[SUCCESS] Executive summary: {output_file}")
        return output_file
...
def main():
    ...
    # ============================================================
    # TEST CONFIGURATION - Adjust these as needed 
    # ============================================================
    compare_with_gbg        = True
    gbg_filter              = None       # None = ALL (approved + rejected) for full comparison
                                         # 'APPROVED' = only GBG-approved records
                                         # 'REJECTED' = only GBG-rejected records
    batch_size              = 5
    # total_limit             = 200        # Increase for more thorough analysis
    total_limit             = 100        # Increase for more thorough analysis

    save_disagreement_images = True      # Set False to skip image saving
    # ============================================================

    print("\n" + "=" * 70)
    print("MAISHA CARD VERIFICATION TEST - AWS vs GBG COMPARISON")
    print("=" * 70)
    print(f"Database:          {oracle_config['dsn']}")
    print(f"API:               {api_base_url}")
    print(f"Batch Size:        {batch_size}")
    print(f"Total Limit:       {total_limit}")
    print(f"GBG Filter:        {gbg_filter if gbg_filter else 'ALL (recommended)'}")
    print(f"AWS Threshold:     {CLIENT_THRESHOLD}% (client-side override)")
    print(f"Save Images:       {save_disagreement_images}")
    print("=" * 70 + "\n")
    ...
    try:
        tester = MaishaVerificationTester(
            ...
            compare_with_gbg=compare_with_gbg,
            client_threshold=CLIENT_THRESHOLD,
            save_disagreement_images=save_disagreement_images
        )
        ...
        # Export deliverables
        csv_file = tester.export_csv()
        exec_summary = tester.generate_executive_summary(analysis)

        skipped_file = None
        if tester.skipped_records:
            skipped_file = tester.export_skipped_records()

        print("=" * 70)
        print("DELIVERABLES:")
        print("=" * 70)
        print(f"1. Detailed CSV:       {csv_file}")
        print(f"2. Executive Summary:  {exec_summary}")
        if skipped_file:
            print(f"3. Skipped Records:    {skipped_file}")
        if save_disagreement_images:
            print(f"4. Images Directory:   {tester.images_dir}/")
            print(f"   ├── 1_GBG_APPROVED_AWS_REJECTED/  (False Negatives - share with AWS team)")
            print(f"   ├── 2_GBG_REJECTED_AWS_APPROVED/  (False Positives - review for fraud)")
            print(f"   ├── 3_BOTH_APPROVED_samples/")
            print(f"   └── 4_BOTH_REJECTED_samples/      (Upload to S3 for AWS team)")
        print(f"5. Log File:           {log_filename}")
        print("=" * 70 + "\n")
